    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                if (args.Count() < 2)
                {
                    Console.WriteLine("[INFO] No template to use specified, using default file.");
                ...
                byte[] sourceFileContent = File.ReadAllBytes(sourceFile);
                byte[] templateFileContent = File.ReadAllBytes(templateFile);

                // ** Fill out the data for the conversion operation.
                ImportFormData inputData = new ImportFormData(
                    sourceFileName: sourceFile,                               // ** Always specify a file name with the correct extension
                    sourceFileContent: sourceFileContent,                          // ** The file content to convert
                    pdfTemplateFileContent: templateFileContent,            // ** Specify the PDF template to import data into
                ...
                var response = convertAPI.ImportFormData(inputData);
