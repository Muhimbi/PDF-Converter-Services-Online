...
if($_FILES["file"]['size'] > 0)
{
    ...
    // ** Set to true for large file conversion.
    $input_data->setUseAsyncPattern(true);
    ...
    try { 
        ...
        // ** When async pattern is used the service returns "Accepted" as the result code
        // ** Polling must be implemented until the result is returned
        if($result->getResultCode() == $result::RESULT_CODE_ACCEPTED) {
            // ** Extract the task id from the response
            $taskId = str_replace("task_id=", "", $result->getResultDetails());

            // ** The async pattern callback endpoint must be used in this case
            $async_pattern_callback = new MuhimbiPDFOnline\Client\Api\AsyncPatternCallbackEndpointsApi();

            while($result->getResultCode() == $result::RESULT_CODE_ACCEPTED) {
                // ** Check job status every 5 seconds
                sleep(5);
                $result = $async_pattern_callback->actionTask($taskId);
            }
        }
...
<html>
  <body>
    <form action="convert_LRO.php" method="post" enctype="multipart/form-data">
