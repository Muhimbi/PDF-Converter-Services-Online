...
if($_FILES["file"]['size'] > 0)
{
    ...
    $input_data = new MuhimbiPDFOnline\Client\Model\ConvertExcelData();
    ...
    try { 
        ...
        $result = $api_instance->convertExcel($input_data);
...
<html>
  <body>
    <form action="sample.php" method="post" enctype="multipart/form-data">
      ...
      &nbsp;&nbsp;&nbsp;
      <input type="file" name="file" accept=".xls,.xlsx,.xlsm,.xlsb,.xlt,.xml,.csv,.dif,.ods,.ots,.mht,.html,.vsdm"/>

      <br/><br>
      <input type="submit" value="Convert Spreadsheet" />
    </form>
  </body>
</html>