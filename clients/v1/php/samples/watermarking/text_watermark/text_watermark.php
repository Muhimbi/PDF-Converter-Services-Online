...
if($_FILES["file"]['size'] > 0)
{
    ...
    $input_data = new MuhimbiPDFOnline\Client\Model\TextWatermarkData();
    ...
    $input_data->setFontStyle('bold|italic');                               // ** One or more styles, e.g. bold|italic|underline|strikeout
    $input_data->setAlignment($input_data::ALIGNMENT_MIDDLE_CENTER);        // ** Alignment of the text within the watermark (not to be confused with 'setPosition')
    $input_data->setWordWrap($input_data::WORD_WRAP_WORD);                  // ** Word wrapping options within the width and height specified for the watermark.
    ...
    $input_data->setWidth(144);                                             // ** The width of the watermark in pts (1/72nd of an inch). For example 144 is 2 inches wide.
    $input_data->setHeight(144);                                            // ** The height of the watermark.
    $input_data->setLayer($input_data::LAYER_FOREGROUND);                   // ** Position the watermark in front of the page's content, or behind it
    $input_data->setRotation(-45);                                          // ** Rotate the watermark, specify a value in degrees. Negative values are accepted.
    ...
    $input_data->setLineWidth(-1);                                          // ** The width of the line in pts (1/72nd of an inch). 0 = hairline, -1 = no line
    ...
    try { 
        ...
        $result = $api_instance->textWatermark($input_data);
...
<html>
  <body>
    <form action="text_watermark.php" method="post" enctype="multipart/form-data">
