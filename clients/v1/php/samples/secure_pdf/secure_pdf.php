...
if($_FILES["file"]['size'] > 0)
{
    ...
    $input_data = new MuhimbiPDFOnline\Client\Model\SecurePdfData();
    ...
    // ** When applying security restrictions (e.g. 'Print|ContentCopy'), always specify an 'owner password', in this example the password is set to 'owner'
    ...
    try { 
        ...
        $result = $api_instance->securePdf($input_data);

        // ** Send the converted file back to the user
        header("Cache-Control: must-revalidate, post-check=0, pre-check=0");
        header("Content-type: application/octet-stream");
        header("Content-Disposition: attachment; filename=\"secured.pdf\"");
...
<html>
  <body>
    <form action="secure_pdf.php" method="post" enctype="multipart/form-data">

      <p>Please specify the PDF file to secure.</p>
      <input type="file" name="file" />

      <br/><br>
      <p>In this example the resulting PDF will require a password to open (the password is 'open') and restrict printing and content copying.</p>
