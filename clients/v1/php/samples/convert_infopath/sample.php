...
if($_FILES["file"]['size'] > 0)
{
    ...
    $input_data = new MuhimbiPDFOnline\Client\Model\ConvertInfopathData();
    ...
    // ** Optionally, pass content of the XSN template to override the one defined by the XML document
    $input_data->setTemplateFileContent(base64_encode(file_get_contents($_FILES["template"]["tmp_name"])));
    ...
    try { 
        ...
        $result = $api_instance->convertInfopath($input_data);
...
<html>
  <body>
    <form action="sample.php" method="post" enctype="multipart/form-data">
      ...
      <br/><br/>
      InfoPath document: <input type="file" name="file" accept=".xml,.infopathxml"/>

      <br/><br/>
      XSN template: <input type="file" name="template" accept=".xsn"/>

      <br/><br/>
      <input type="submit" value="Convert InfoPath Document" />
    </form>
  </body>
</html>