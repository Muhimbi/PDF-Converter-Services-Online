...
if($_FILES["file"]['size'] > 0)
{
    ...
    $input_data = new MuhimbiPDFOnline\Client\Model\ConvertVisioData();
    ...
    try { 
        ...
        $result = $api_instance->convertVisio($input_data);
...
<html>
  <body>
    <form action="sample.php" method="post" enctype="multipart/form-data">
      ...
      &nbsp;&nbsp;&nbsp;
      <input type="file" name="file" accept=".vsd,.vdx,.svg,.svgz,.vdw,.vsdx,.vss,.vssx,.vst,.vstx,.vsdm"/>

      <br/><br>
      <input type="submit" value="Convert Visio document" />
    </form>
  </body>
</html>