...
public class sample {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            byte[] sourceFile = Files.readAllBytes(new File("sample.xls").toPath());

            // ** Fill out all data for this operation
            ConvertExcelData inputData = new ConvertExcelData();
            inputData.setSourceFileName("sample.xls");                          // ** Always specify a file name with the correct extension
            inputData.setSourceFileContent(encode(sourceFile));                 // ** The file content to process, make sure it is base 64 encoded
            inputData.setOutputFormat(ConvertExcelData.OutputFormatEnum.PDF);   // ** The format to convert the file to (not all combinations are supported)
            ...
            OperationResponse result = apiInstance.convertExcel(inputData);
