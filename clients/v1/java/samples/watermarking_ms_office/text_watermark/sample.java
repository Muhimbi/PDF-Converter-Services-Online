...
public class sample {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            byte[] sourceFile = Files.readAllBytes(new File("test.pptx").toPath());

            // ** Fill out all data for this operation
            TextWatermarkData inputData = new TextWatermarkData();
            inputData.setSourceFileName("test.pptx");                                       // ** Always specify a file name with the correct extension (file must be a PDF, docx, xlsx or pptx file)
            inputData.setSourceFileContent(encode(sourceFile));                             // ** The file content to watermark
            inputData.setPosition(TextWatermarkData.PositionEnum.MIDDLE_CENTER);            // ** The location where the watermark will be positioned on the page.
            inputData.setContent("Hello World");                                            // ** The Text to include
            inputData.setFontFamilyName("Times New Roman");                                 // ** Name of the font, e.g. Arial, Times New Roman, Calibri
            inputData.setFontSize("24");                                                    // ** Size of the font in Pt
            inputData.setFontStyle("bold|italic");                                          // ** One or more styles, e.g. bold|italic|underline|strikeout
            inputData.setAlignment(TextWatermarkData.AlignmentEnum.MIDDLE_CENTER);          // ** Alignment of the text within the watermark (not to be confused with 'setPosition')
            inputData.setWordWrap(TextWatermarkData.WordWrapEnum.WORD);                     // ** Word wrapping options within the width and height specified for the watermark.
            ...
            inputData.setLayer(TextWatermarkData.LayerEnum.FOREGROUND);                     // ** Position the watermark in front of the page's content, or behind it
            inputData.setRotation("-45");                                                   // ** Rotate the watermark, specify a value in degrees. Negative values are accepted.
            ...
            inputData.setLineWidth("-1");                                                   // ** The width of the line in pts (1/72nd of an inch). 0 = hairline, -1 = no line
            ...
            inputData.setPageOrientation(TextWatermarkData.PageOrientationEnum.BOTH);       // ** The page orientation the watermark applies to. 

            // ** Carry out the operation
            System.out.println("[INFO] Processing...");
            OperationResponse result = apiInstance.textWatermark(inputData);

            // ** Write the results back to the file system
            FileOutputStream fos = new FileOutputStream("result.pptx");
            fos.write(decode(result.getProcessedFileContent()));
            fos.close();

            System.out.println("[INFO] 'result.pptx' written to output folder.");
