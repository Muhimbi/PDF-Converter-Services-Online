...
public class sample {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            byte[] sourceFile = Files.readAllBytes(new File("sample.doc").toPath());

            // ** Fill out all data for this operation
            ConvertWordData inputData = new ConvertWordData();
            inputData.setSourceFileName("sample.doc");                             // ** Always specify a file name with the correct extension
            inputData.setSourceFileContent(encode(sourceFile));                     // ** The file content to process, make sure it is base 64 encoded
            inputData.setOutputFormat(ConvertWordData.OutputFormatEnum.PDF);  // ** The format to convert the file to (not all combinations are supported)
            ...
            OperationResponse result = apiInstance.convertWord(inputData);
