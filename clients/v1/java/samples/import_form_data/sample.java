...
public class sample {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            byte[] sourceFile = Files.readAllBytes(new File("sample.xfdf").toPath());
            byte[] templateFile = Files.readAllBytes(new File("sample.pdf").toPath());

            // ** Fill out all data for this operation
            ImportFormData inputData = new ImportFormData();
            inputData.setSourceFileName("sample.xfdf");                 // ** Always specify a file name with the correct extension
            inputData.setSourceFileContent(encode(sourceFile));         // ** The file content to process, make sure it is base 64 encoded
            inputData.setPdfTemplateFileContent(encode(templateFile));  // ** Set the PDF template file as well to import the data into
            ...
            OperationResponse result = apiInstance.importFormData(inputData);
