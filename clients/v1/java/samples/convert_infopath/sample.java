...
public class sample {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            // ** Read the source file and template file we wish to process
            byte[] sourceFile = Files.readAllBytes(new File("sample.xml").toPath());
            byte[] templateFile = Files.readAllBytes(new File("sample.xsn").toPath());

            // ** Fill out all data for this operation
            ConvertInfopathData inputData = new ConvertInfopathData();
            inputData.setSourceFileName("sample.xml");                              // ** Always specify a file name with the correct extension
            inputData.setSourceFileContent(encode(sourceFile));                     // ** The file content to process, make sure it is base 64 encoded
            inputData.setTemplateFileContent(encode(templateFile));                 // ** Optionally, set the XSN template file as well
            inputData.setOutputFormat(ConvertInfopathData.OutputFormatEnum.PDF);    // ** The format to convert the file to (not all combinations are supported)
            ...
            OperationResponse result = apiInstance.convertInfopath(inputData);
