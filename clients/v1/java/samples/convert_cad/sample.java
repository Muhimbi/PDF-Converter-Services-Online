...
public class sample {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            byte[] sourceFile = Files.readAllBytes(new File("sample.dwg").toPath());

            // ** Fill out all data for this operation
            ConvertCadData inputData = new ConvertCadData();
            inputData.setSourceFileName("sample.dwg");                                          // ** Always specify a file name with the correct extension
            inputData.setSourceFileContent(encode(sourceFile));                                 // ** The file content to process, make sure it is base 64 encoded
            inputData.setForegroundColor(ConvertCadData.ForegroundColorEnum.GREYSCALEDARKEN);   //** Make foreground grayscale and darken a bit for better visibility
            ...
            OperationResponse result = apiInstance.convertCad(inputData);
