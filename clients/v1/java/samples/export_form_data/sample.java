...
public class sample {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            byte[] sourceFile = Files.readAllBytes(new File("sample.pdf").toPath());

            // ** Fill out all data for this operation
            ExportFormData inputData = new ExportFormData();
            inputData.setSourceFileName("sample.pdf");                          // ** Always specify a file name with the correct extension
            inputData.setSourceFileContent(encode(sourceFile));                 // ** The file content to process, make sure it is base 64 encoded
            inputData.setOutputFormat(ExportFormData.OutputFormatEnum.XFDF);    // ** The format to convert the file to (not all combinations are supported)

            // ** Carry out the operation
            System.out.println("[INFO] Processing...");
            OperationResponse result = apiInstance.exportFormData(inputData);

            // ** Write the results back to the file system
            FileOutputStream fos = new FileOutputStream("result.xfdf");
            fos.write(decode(result.getProcessedFileContent()));
            fos.close();

            System.out.println("[INFO] 'result.xfdf' written to output folder.");
