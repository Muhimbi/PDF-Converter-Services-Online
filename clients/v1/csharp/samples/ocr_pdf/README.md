...
Once the sample has completed the OCR process, it will attempt to launch the generated PDF in the system's default PDF Viewer. If this is not configured (or not possible under Linux) then the generated PDF file can be found in the `bin/Debug` folder.

Once opened in a viewer, it should now be possible select text or search for words. 
